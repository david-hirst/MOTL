...
Point to correct package library?
...
```{r}
EvaluationsDF$fctrzn_type[EvaluationsDF$fctrzn_type=="ProjectR_CS"]="projectR"
...
Boxplots for F1 scores - all configs, excluding MCMC scores
...
DFToPlot$fctrzn_grp = DFToPlot$fctrzn_type
...
DFToPlot$fctrzn_type = as.factor(DFToPlot$fctrzn_type)
DFToPlot$fctrzn_type = relevel(DFToPlot$fctrzn_type, ref = "Direct")
...
ggplot(DFToPlot, aes(x=fctrzn_type, y=score_adj, fill=fctrzn_grp))+ 
  ylab(PlotScore) +
  xlab("") +
  # geom_violin() +
...
DFToScatterPlot = DFToScatterPlot[DFToScatterPlot$M == 3
                                  & EvaluationsDF$D==2000
                                  & DFToScatterPlot$fctrzn_type != "Random_TL"
                                  ,]
...
knitr::kable(SmryStatsWide, 
             digits=c(0,0,0,0,1,2,2,2,2,2), booktabs = T, row.names=F) %>%
  kable_styling(full_width = F, 
                font_size = 12)%>%
  save_kable(file.path(BaseDir,paste0(TableScore,"Table.png")))
